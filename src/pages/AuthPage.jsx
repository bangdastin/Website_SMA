...
import { Mail, Lock, User, Hash, Eye, EyeOff, ArrowLeft, CheckCircle2, AlertCircle, Loader2, KeyRound, CalendarOff, Home, Send } from 'lucide-react';
...
const AuthPage = ({ initialView = 'login' }) => {
  ...
  const [view, setView] = useState(urlView === 'reset' && urlToken ? 'reset' : initialView); 
  ...
  const handleReset = async (e) => {
    ...
    if (!urlToken) return setMessage({ type: 'error', text: 'Token tidak valid atau kadaluarsa.' });

    setIsLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/auth/reset-password`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: urlToken, newPassword: resetData.newPassword })
      });
      const data = await res.json();
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4 py-10 font-sans relative overflow-hidden">
      ...
      <div className={`bg-white w-full max-w-md p-8 rounded-2xl shadow-xl border border-slate-100 relative animate-fade-in transition-all duration-500 ${successModal.show ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
        ...
        {/* TOMBOL KEMBALI */}
        {view !== 'login' ? (
            // Jika BUKAN Login, kembali ke Login
            <button onClick={() => setView('login')} className="absolute top-6 left-6 text-slate-400 hover:text-blue-600 transition-colors p-2 hover:bg-slate-50 rounded-full" title="Kembali">
                <ArrowLeft size={24} />
            </button>
        ) : (
            // JIKA LOGIN, kembali ke Home (Landing Page)
            <button onClick={() => navigate('/')} className="absolute top-6 left-6 text-slate-400 hover:text-blue-600 transition-colors p-2 hover:bg-slate-50 rounded-full" title="Kembali ke Beranda">
                <ArrowLeft size={24} /> {/* <--- BAGIAN INI YANG DIGANTI (Dulunya <Home />) */}
            </button>
        )}
        
        <div className="text-center mb-8 mt-4">
          <div className="inline-block p-3 bg-blue-50 text-blue-600 rounded-2xl mb-4 shadow-sm">
            {view === 'login' ? <User size={32} /> : view === 'register' ? (regStatus === 'closed' ? <CalendarOff size={32} className="text-red-500"/> : <User size={32} />) : view === 'forgot' ? <Mail size={32} /> : <KeyRound size={32} />}
          </div>
          <h2 className="text-2xl font-bold text-slate-800 tracking-tight">
            {view === 'login' ? 'Selamat Datang!' : view === 'register' ? (regStatus === 'closed' ? 'Pendaftaran Ditutup' : 'Buat Akun Siswa') : view === 'forgot' ? 'Lupa Password?' : 'Reset Password'}
          </h2>
          <p className="text-slate-500 text-sm mt-2">
            {view === 'login' ? 'Masuk ke portal pendaftaran siswa baru.' : view === 'register' ? (regStatus === 'closed' ? '' : 'Isi data diri Anda untuk mendaftar.') : view === 'forgot' ? 'Kami akan mengirimkan link reset ke email Anda.' : 'Buat password baru yang aman.'}
          </p>
        </div>
        ...
        {/* --- FORM REGISTER / CLOSED MESSAGE --- */}
        {view === 'register' && (
          <>
            {regStatus === 'closed' ? (
                // TAMPILAN JIKA PENDAFTARAN DITUTUP
                <div className="flex flex-col items-center justify-center animate-fade-in py-4">
                    <div className="bg-red-50 text-red-600 p-4 rounded-full mb-4 ring-4 ring-red-50/50 animate-pulse">
                        <CalendarOff size={48} />
                    </div>
                    <p className="text-center text-slate-600 font-medium mb-6 leading-relaxed px-2">
                        Mohon maaf, saat ini <span className="font-bold text-slate-800">Pendaftaran Peserta Didik Baru</span> sedang ditutup. 
                        Silahkan mencoba kembali atau mendaftar di periode yang akan datang.
                    </p>
                    <div className="space-y-3 w-full">
                        <button onClick={() => setView('login')} className="w-full bg-blue-600 text-white font-bold py-3 rounded-xl shadow-lg hover:bg-blue-700 transition-all active:scale-95">
                            Kembali ke Login
                        </button>
                        <button onClick={() => navigate('/')} className="w-full bg-slate-100 text-slate-600 font-bold py-3 rounded-xl hover:bg-slate-200 transition-all flex items-center justify-center gap-2">
                            <Home size={18}/> Kembali ke Beranda
                        </button>
                    </div>
                </div>
            ) : (
                // FORMULIR PENDAFTARAN (JIKA DIBUKA)
        ...
        {/* --- VIEW 3: FORGOT PASSWORD (DITAMBAHKAN) --- */}
        {view === 'forgot' && (
            <form onSubmit={handleForgot} className="space-y-6 animate-fade-in">
                <div className="bg-blue-50 p-4 rounded-xl text-sm text-blue-700 border border-blue-100 flex gap-3 items-start">
                    ...
                    <span>Masukkan email yang terdaftar. Kami akan mengirimkan tautan untuk mengatur ulang kata sandi Anda.</span>
                </div>
                <div className="relative group">
                ...
                <div className="space-y-3">
                    <button disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-xl shadow-lg flex justify-center items-center gap-2 transition-all active:scale-95 disabled:opacity-70">
                        {isLoading ? <Loader2 size={20} className="animate-spin"/> : <><Send size={18}/> Kirim Link Reset</>}
                    </button>
                    <button type="button" onClick={() => setView('login')} className="w-full py-3 rounded-xl text-slate-500 font-semibold hover:bg-slate-50 transition-colors">
                        Kembali ke Login
                    </button>
                </div>
            </form>
        ...
        {view === 'reset' && (
            <form onSubmit={handleReset} className="space-y-4 animate-fade-in">
                ...
                <button disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-xl shadow-lg mt-2 flex justify-center items-center gap-2 transition-all active:scale-95 disabled:opacity-70">
